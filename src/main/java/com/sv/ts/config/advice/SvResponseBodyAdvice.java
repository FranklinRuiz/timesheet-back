...
public abstract class SvResponseBodyAdvice implements ResponseBodyAdvice<Object> {
    ...
    public Object beforeBodyWrite(@Nullable Object body, MethodParameter returnType, MediaType selectedContentType, Class<? extends HttpMessageConverter<?>> selectedConverterType, ServerHttpRequest request,
        ...
        if (request.getURI().getPath().startsWith("/api/") && !(body instanceof SvResponseEntity) && !(body instanceof InputStreamResource)) {
            @SuppressWarnings("rawtypes")
            SvResponseEntity icaaResponseEntity = new SvResponseEntity();
